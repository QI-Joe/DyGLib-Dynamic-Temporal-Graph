    ...
    for run in range(views):
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                batch_lable_mask = train_data.true_seen_label_mask[:, train_data_indices]
                ...
                batch_label_mask = batch_lable_mask.flatten()
                predicts = F.softmax(torch.vstack([src_predicts, dst_predicts]), dim=-1)[batch_label_mask]
                labels = torch.from_numpy(batch_labels).to(predicts.device)[batch_label_mask]
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                ...
                new_node_test_loss, new_node_test_metrics = evaluate_model_node_classification(model_name=args.model_name,
                                                                                               model=model,
                                                                                               neighbor_sampler=test_neighbor_sampler,
